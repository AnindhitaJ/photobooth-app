(function () {
  ...
  function drawCertificate(state, theme) {
    const ctx = state.ctx, t = state.text, c = theme.colors;
    bg(ctx, c);
    ctx.fillStyle='rgba(255,255,255,.72)'; roundRect(ctx,70,80,W-140,H-160,46); ctx.fill();
    ctx.strokeStyle=c[1]; ctx.lineWidth=8; roundRect(ctx,70,80,W-140,H-160,46); ctx.stroke();
    ctx.strokeStyle=c[2] || c[1]; ctx.lineWidth=2; roundRect(ctx,105,115,W-210,H-230,34); ctx.stroke();
    deco(ctx, theme, .35);

    ctx.fillStyle = c[3] || '#111827';
    ctx.textAlign='center'; ctx.textBaseline='top';
    fitFont(ctx, t.title, 920, 60, 30, '950', 'Georgia, Times New Roman, serif');
    ctx.fillText(t.title || '', W/2, 160);
    ctx.fillStyle=c[1]; ctx.font='800 28px Segoe UI, sans-serif';
    ctx.fillText(t.subtitle || '', W/2, 250);

    const mask = theme.id.includes('love') || theme.id === 'cutest' ? 'circle' : 'rect';
    if (mask === 'circle') {
      ctx.save();
      ctx.beginPath(); ctx.ellipse(W/2, 520, 245, 245, 0, 0, Math.PI*2); ctx.clip();
      drawPhoto(ctx,state,W/2-245,275,490,490,0);
      ctx.restore();
      ctx.strokeStyle=c[1]; ctx.lineWidth=8; ctx.beginPath(); ctx.ellipse(W/2,520,245,245,0,0,Math.PI*2); ctx.stroke();
    } else {
      drawPhoto(ctx,state,315,310,550,440,32);
      ctx.strokeStyle=c[1]; ctx.lineWidth=7; roundRect(ctx,315,310,550,440,32); ctx.stroke();
    }

    ctx.fillStyle = c[3] || '#111827';
    fitFont(ctx, t.name, 850, 66, 32, '950', 'Georgia, Times New Roman, serif');
    ctx.fillText(t.name || '', W/2, 820);
    ctx.font='700 31px Segoe UI, sans-serif';
    drawWrapped(ctx, t.description, W/2, 925, 760, 42, 3, 'center');

    drawSeal(ctx, 875, 1230, 116, t.stamp, c[1]);
    ctx.strokeStyle=c[1]; ctx.lineWidth=3;
    ctx.beginPath(); ctx.moveTo(175,1328); ctx.lineTo(475,1328); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(690,1328); ctx.lineTo(990,1328); ctx.stroke();
    ctx.fillStyle=c[3] || '#111827';
    ctx.font='800 25px Segoe UI, sans-serif'; ctx.textAlign='center';
    ctx.fillText(t.date || '', 325, 1348);
    ctx.fillText(t.signature || '', 840, 1348);
    ctx.font='700 18px Segoe UI, sans-serif';
    ctx.fillText('DATE', 325, 1384);
    ctx.fillText('SIGNATURE', 840, 1384);
  }
